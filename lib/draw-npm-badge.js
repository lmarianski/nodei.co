...
function drawNpmInstall (ctx, margin, name, style, opts) {
  ctx.font = '14px ubuntu-b'
  ctx.fillStyle = 'rgb(102, 102, 102)'
  ctx.textBaseline = 'top'
  ctx.fillText(
      `npm install ${opts ? opts+" " : ""}${name}`
    , margin + (style == 'mini'
        ? 50
        : style == 'compact'
          ? 86
...
function calculateParams (options, pkginfo) {
  ...
  return {
    ...
    , installOpts   : options.install === "save"     ? "--save" : 
                      options.install === "save-dev" ? "--save-dev" :
                      options.install === "global"   ? "-g" :
                      options.install === "none"     ? "none" : null
  }
}
...
function draw (options, pkginfo) {
  ...
  if (params.installOpts !== "none") {
    drawNpmInstall(ctx, params.margin, params.installName, params.style, params.installOpts)
  }
